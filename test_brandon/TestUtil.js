...
class TestUtil {
    static verifyQueryResult(futureResult, test) {
        ...
        if (test.isQueryValid) {
            ...
            const err = test.result === "ResultTooLargeError"
                ? IInsightFacade_1.ResultTooLargeError
                : IInsightFacade_1.InsightError;
            expectation = chai_1.expect(futureResult).to.be.rejectedWith(err);
        }
        return expectation;
    }
    static readTestQueries(path = "test_brandon/queries") {
